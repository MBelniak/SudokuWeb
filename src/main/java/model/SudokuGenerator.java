...
public class SudokuGenerator {
     ...
     * @see SudokuCell
     */
    private static final HashSet<Integer> VALID_NUMBERS = new HashSet<>(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9));
    ...
    boolean doFillCells(int index) {
        ...
        cell.generateAllNeighbors(this.sudokuList);

        HashSet<Integer> neighborValues = new HashSet<>();

        for (SudokuCell neighbor : cell.neighbors) {
            int value = neighbor.getFixedValue();
            neighborValues.add(value);
        }

        ArrayList<Integer> options = new ArrayList<>(SudokuGenerator.VALID_NUMBERS);
        options.removeAll(neighborValues);
        Collections.shuffle(options);

        for (int option : options) {

            cell.setFixedValue(option);
     ...
     * Values in particular fields can be earlier set using {@link SudokuCell#getCellByCoords(LinkedList, int[])} method.
    ...
    boolean doFillCellsUnique(int index) {
        ...
        for (int option : options) {
            ...
            {
                if (sudokuList.get(i).getFixedValue() == 0)   //so if the cell's value wasn't given, let's fill it
